...
describe("Fast mode", () => {
	...
	it("supports Cursor Grok without enabling fast for Cursor Kimi", () => {
		expect(supportsFastMode(model("cursor-agent", "cursor-grok-4.5-high", "cursor-agent"))).toBe(true);
		expect(supportsFastMode(model("cursor-agent", "kimi-k3-max", "cursor-agent"))).toBe(false);
