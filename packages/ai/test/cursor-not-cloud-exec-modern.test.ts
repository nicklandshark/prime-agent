...
import { kCursorExecResolved, setStreamingPartialJson } from "../src/utils/cursor-not-cloud-blocks.js";
...
describe("Cursor exec server control", () => {
	it("cancels the specific provider-owned execution and returns a bounded typed result", async () => {
		const output = cursorAssistantMessage();
		const stream = new AssistantMessageEventStream();
		const state = newBlockState();
		const written: Buffer[] = [];
		...
		const execMessage = buildExecMessage({
			case: "readArgs",
			value: create(ReadArgsSchema, { path: "slow.txt", toolCallId: "abort-read" }),
		});
		...
		const execPromise = handleServerMessage(
			...
			h2Request,
			{
				read: async (_args, context) => {
					markStarted?.();
					return await new Promise<ToolResultMessage>((_resolve, reject) => {
						const abort = () => {
							observedAbort = true;
							reject(new Error("read cancelled by server"));
						};
		...
		const frames = written.map(decodeClientFrame);
		expect(frames).toHaveLength(1);
		expect(frames[0].message.case).toBe("execClientMessage");
		if (frames[0].message.case !== "execClientMessage") throw new Error("wrong response frame");
