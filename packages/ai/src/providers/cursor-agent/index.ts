...
} from "../../utils/cursor-agent-blocks.js";
...
export function disposeCursorAgentConversation(conversationId: string): boolean {
	return cursorConversationStateStore.disposeConversation(conversationId);
}
...
export const streamCursor: StreamFunction<"cursor-agent", CursorAgentOptions> = (
	...
	(async () => {
		...
		try {
			...
			conversationLease = cursorConversationStateStore.acquire(conversationId);
...
export async function handleServerMessage(
	...
	if (msgCase === "interactionUpdate") {
		...
		log("warn", "unhandledAgentServerMessage", { msgCase });
	}
}
...
export function resolveCursorAgentModelId(
	...
	const normalModelId = typeof mapped === "string" ? mapped : (model.requestModelId ?? model.id);
	if (options?.serviceTier !== "priority") return normalModelId;
	return CURSOR_GROK_FAST_ROUTES[normalModelId] ?? normalModelId;
}

/** Provider-agnostic option mapping plus Cursor sibling-model routing. */
export const streamSimpleCursorAgent: StreamFunction<"cursor-agent", SimpleStreamOptions> = (
	model,
	context,
	options,
): AssistantMessageEventStream => {
