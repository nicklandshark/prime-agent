...
import { createCursorDeleteTool } from "../src/core/cursor-not-cloud/delete-tool.js";
...
describe("Cursor Agent exec bridge", () => {
	...
	it("isolates observer failures and executes a side-effecting tool exactly once", async () => {
		let executions = 0;
		const tool = fakeTool("mcp__demo__mutate_once", () => executions++);
		const handlers = createCursorExecHandlers({
			getTools: () => [tool],
			emitEvent: async () => {
		...
		const result = await handlers.mcp?.({
			...
			providerIdentifier: "prime-agent",
			toolName: tool.name,
			toolCallId: "mcp-observer-failure",
			args: { mutation: true },
			rawArgs: {},
		});
		expect(executions).toBe(1);
		expect(result).toMatchObject({ toolCallId: "mcp-observer-failure", isError: false });
	});
